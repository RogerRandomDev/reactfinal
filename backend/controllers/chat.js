...
const getMessages = async (user1, user2, product) => {
  try {
    ...
    let list = await MessageModel.find({
      $or: [
        { sender: h1, receiver: h2, product },
        { receiver: h1, sender: h2, product },
      ],
    })
      .sort({ _id: -1 })
      .limit(50);
    return {
      success: true,
      msg: 'succeeded at getting user messaged with other user',
      list,
    };
  } catch (err) {}
  return {
    success: false,
    msg: 'failed to obtain message list with other user',
  };
};
...
const getConversations = async (user) => {
  ...
  try {
    ...
    return {
      success: true,
      msg: 'successfully got all unique message lists',
      list,
    };
  } catch (err) {}
