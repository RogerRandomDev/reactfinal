    ...
    searchValue.length===0 || dataSet["profile"].toLowerCase().includes(searchValue.toLowerCase()) ? <tr key={idx} className={`${checked || 'hover:bg-slate-200'} transition text-center ${checked && "bg-blue-200"}`}>
                  {Object.keys(dataSet).map((key,index)=>{
                    if(key==="checkbox"){
                      ...
                      return <td className="p-4" key={index}><div className="flex gap-8 items-center justify-center"><img src="https://picsum.photos/200" alt={"Random Profile"} className="rounded-full w-8 h-8 object-cover"/>{dataSet[key]}</div></td>
                    } else if(key==="items"){
