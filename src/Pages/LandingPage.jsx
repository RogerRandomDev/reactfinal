...
function LandingPage({ updateContext }) {
  ...
  const formInputReducer = (state, action) => {
    if (action.type === "changeToUserType") {
      ...
      landingSignup.current.classList.add("overflow-hidden");
      if (state.isSignUp) {
        mover.current.classList.add("md:translate-x-[0%]");
        mover.current.classList.remove("md:-translate-x-[70%]");
        landingSignup.current.classList.remove("h-auto")
      }
    ...
    if (action.type === "changeToBusinessType") {
      ...
      landingSignup.current.classList.remove("overflow-hidden");
      if (state.isSignUp) {
        mover.current.classList.add("md:-translate-x-[70%]");
        mover.current.classList.remove("md:translate-x-[0%]");
        landingSignup.current.classList.add("h-auto")
      }
 ...
 const handleSubmit = async (e) =>{
  ...
  return (
    <section ref={section} className="">
      <div className="block md:flex h-screen -mt-14 relative transition delay-200 duration-[600ms] ease-in-out" ref={mover}>

        <div ref={hero} className="p-1 w-full md:w-[70%] text-center block md:flex flex-col justify-center mt-32 md:mt-0 md:mb-0">
          <img src={require("../assets/testingsvg.PNG")} className="max-w-full object-cover w-9/12 mx-auto mb-4" alt="" />
          <h2 className='text-4xl font-semibold mb-4 text-blue-900'>Software Analytics and Marketing Statistics</h2>
        ...
        <div ref={landingSignup} className="overflow-hidden relative landing__signup w-full md:w-[30%] bg-blue-900 block md:flex flex-col items-center justify-between py-12 md:pt-36 px-12 h-[33rem] md:h-auto text-center text-neutral-100 border-r border-blue-900">
...
visible={true}
 />
          ...
          <div ref={businessInfoSlider} className="md:absolute top-0 h-screen md:w-[70.1vw] md:translate-x-[49.05vw] bg-blue-900 flex flex-col items-center justify-center gap-10">
